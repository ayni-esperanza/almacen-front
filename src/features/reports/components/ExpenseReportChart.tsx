          ...
          <div className="relative w-64 h-64 mx-auto">
            <svg viewBox="0 0 100 100" className="transform -rotate-90">
              {data.map((item, index) => {
                ...
                const x2 =
                  50 + 45 * Math.cos(((startAngle + angle) * Math.PI) / 180);
                const y2 =
                  50 + 45 * Math.sin(((startAngle + angle) * Math.PI) / 180);
            ...
            {/* Tooltip flotante */}
            {hoveredIndex !== null && (
              <div className="absolute px-3 py-2 -translate-x-1/2 -translate-y-1/2 bg-white border border-gray-200 rounded-lg shadow-xl left-1/2 top-1/2 dark:bg-slate-800 dark:border-slate-700">
                <div className="space-y-1 text-center">
                  ...
                  <div className="text-xs text-gray-500 dark:text-slate-400">
                    {((data[hoveredIndex].gasto / total) * 100).toFixed(1)}% del
                    total
                  </div>
                </div>
              </div>
              ...
              return (
                ...
                >
                  <div className="flex items-center flex-1 space-x-3">
                    <div
                      className={`h-4 w-4 rounded-full flex-shrink-0 transition-transform ${
                        isHovered ? "scale-110" : ""
                      }`}
              ...
              return (
                <g key={index}>
                  <circle
                    ...
                    r={isHovered ? "8" : "5"}
                    fill="#10b981"
                    className="transition-all cursor-pointer dark:fill-emerald-500"
                    onMouseEnter={() => setHoveredIndex(index)}
                    onMouseLeave={() => setHoveredIndex(null)}
                  />
                  {isHovered && (

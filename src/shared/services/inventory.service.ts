...
class InventoryService {
  async getAllProducts(search?: string): Promise<Product[]> {
    const endpoint = search ? `/inventory/products?search=${encodeURIComponent(search)}` : '/inventory/products';
  ...
  async createProduct(productData: CreateProductData): Promise<Product | null> {
    const response = await apiClient.post<Product>('/inventory/products', productData);
  ...
  async updateProduct(id: string, productData: UpdateProductData): Promise<Product | null> {
    const response = await apiClient.patch<Product>(`/inventory/products/${id}`, productData);
  ...
  async getAreas(): Promise<string[]> {
    const response = await apiClient.get<{nombre: string}[]>('/inventory/areas');

...
class EquipmentService {
  ...
  async getEquipmentByCode(
    serieCodigo: string
  ): Promise<EquipmentReport | null> {
    const response = await apiClient.get<EquipmentReport>(
      `/equipment/code/${encodeURIComponent(serieCodigo)}`
    );

    if (response.error) {
      // No mostrar error en consola si no se encuentra
      if (!response.error.includes("404")) {
        console.error("Error fetching equipment by code:", response.error);
      }
      return null;
    }
  ...
  async createEquipmentReport(
    equipmentData: CreateEquipmentData
  ): Promise<EquipmentReport | null> {
    const response = await apiClient.post<EquipmentReport>(
      "/equipment",
      equipmentData
    );
  ...
  async updateEquipment(
    id: string,
    equipmentData: UpdateEquipmentData
  ): Promise<EquipmentReport | null> {
    const response = await apiClient.patch<EquipmentReport>(
      `/equipment/${id}`,
      equipmentData
    );
